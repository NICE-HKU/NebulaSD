...
def _current_cuda_device_name() -> str:
...
class LlamaTransformerLayerWeight(WeightBase):
    ...
    def __init__(
        ...
        self.register_weight(RegisteredWeightItem(
            ...
            (self.model_config.num_q_heads * self.model_config.head_dim, self.model_config.hidden_size),
            self.dtype
        ))
        self.register_weight(RegisteredWeightItem(
            "k_proj",
        ...
        if getattr(self.model_config, "has_qk_norm", False):
            self.register_weight(RegisteredWeightItem(
                "q_norm",
                f"model.layers.{self.layer_id}.self_attn.q_norm.weight",
                (self.model_config.head_dim,),
                self.dtype
            ))
            self.register_weight(RegisteredWeightItem(
                "k_norm",
                f"model.layers.{self.layer_id}.self_attn.k_norm.weight",
                (self.model_config.head_dim,),
                self.dtype
            ))
        self.register_weight(RegisteredWeightItem(
            ...
            (self.model_config.hidden_size, self.model_config.num_q_heads * self.model_config.head_dim),
            self.dtype
        ))
...
def load_weights(
    ...
    if use_dummy:
        def weight_getter_dummy(item: RegisteredWeightItem):
            return torch.empty(item.shape, dtype=item.dtype, device=_current_cuda_device_name()).uniform_(-0.001, 0.001)
        ...
        if len(safetensor_files) > 0:
            ...
            def weight_getter_real(item: RegisteredWeightItem):
                ...
                with safetensors.safe_open(file_path, framework="pt", device=_current_cuda_device_name()) as f:
            ...
            def weight_getter_real(item: RegisteredWeightItem):
                ...
                if file_path not in opened_files:
                    opened_files[file_path] = torch.load(file_path, map_location=_current_cuda_device_name(), mmap=True)
